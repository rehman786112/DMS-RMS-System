...
class Vendors(QWidget):
    ...
    def load_companies(self):
        try:
            rows = self.execute_query("SELECT DISTINCT vnd_company FROM vendors WHERE vnd_company IS NOT NULL AND vnd_company != '' ORDER BY vnd_company")
            self.company_data = [str(r['vnd_company']) for r in rows]
        except Exception as e:
    ...
    def save_type_data(self, checked=False):
        self.add_type_widget = AddDetails('vendor_type')
    ...
    def receive_vendor_data(self, data):
        ...
        vendor_data = self.execute_query(
    ...
    def save_data(self, checked=False):
        ...
        if vendor_type == self.TYPE_PLACEHOLDER or not vendor_type:
        ...
        if self.is_update:
            query = """
                UPDATE vendors SET 
                    vnd_name=%s, vnd_email=%s, vnd_address=%s,
                    vnd_city=%s, vnd_country=%s, vnd_phone=%s,
                    vnd_whatsapp=%s, vnd_type=%s, vnd_cnic=%s,
                    vnd_date=%s, vnd_company=%s, vnd_credit_limit=%s
                WHERE vnd_code=%s
            """
            values = values_common + (self.vendor_code,)
    ...
    @handle_errors
    def refresh_vendor(self, checked=False):
        self.current_date = datetime.now().strftime("%d-%m-%y")
        self.load_types()
        self.load_companies()
        self.update_company_combo(preserve_selection=False)
